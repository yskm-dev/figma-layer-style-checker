...
function postProcessReasons(reasons, suggestedStyles) {
    ...
    const hasDetail = reasons.some((reason) => typeof reason.detail === 'string' && reason.detail.length > 0);
    const hasTextMixedReason = reasons.some((reason) => reason.label === 'Text style が混在');
