...
- 選択レイヤーがある場合のみスキャン: 選択レイヤー自身 + 子孫ノード
- 選択レイヤーがない場合: スキャン非実行（UI に未選択ステートを表示）
...
- デフォルト: ライトテーマ（白背景 + ブルーアクセント）
- ダークテーマ: `html.dark` クラスで CSS 変数を上書き切替
- 切替: フッター右端のアイコンボタン（☽ / ☀）押下

### 8.1 ヘッダー操作

- アプリタイトル（左端）
- `スキャン`（solid ブルーボタン）
- `一括適用（n）`（候補があるレイヤー数を括弧内に表示）
...
- 未スキャン: 「未スキャン」ステート（起動直後）
- 未選択: 「未選択」ステート（選択なしでスキャン試行時）
- 問題あり: 「問題のあるレイヤー n件」
- 全解消: 「問題はすべて解消済み n件」
- 0件: 「問題なし」ステート
...
- **自動スキャントグル**: チェックボックス型トグルスイッチ。デフォルト ON。ON 時はレイヤー選択変更のたびに自動スキャン実行
- **テーマ切替ボタン**: ライトモード時は月アイコン（ダークへ切替）、ダークモード時は太陽アイコン（ライトへ切替）
...
- 手動: UI の `scan` 受信時にスキャン
  - 選択なし時は `no-selection` を UI へ送信し、スキャンは実行しない
- 自動: `autoScanEnabled === true` のとき、`selectionchange` ごとにスキャン
  - 選択なし時は何もしない
- スキャン実行中に自動トリガーが重なった場合は、保留フラグで1回再実行する
- 起動時: UI から `set-auto-scan`（`initial: true` 付き）を受信して状態を同期するが、スキャンは実行しない
...
- `scan`
- `set-auto-scan`（`enabled: boolean`, `initial?: boolean`）
- `cancel`
- `focus-node`
- `apply-style`
- `apply-bulk`
...
- スキャン完了（手動実行時のみ）: `スキャン完了（選択範囲）: 該当レイヤー {n}件`
- 個別適用成功: `スタイルを適用しました`
